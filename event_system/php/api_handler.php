...
switch ($action) {
    ...
    case 'mark_attendance':
        ...
        // Lookup user
        $stmt = $conn->prepare("SELECT id, name FROM users WHERE reg_id=? LIMIT 1");
        ...
        // Check-In
        if ($type === 'in') {
            $chk = $conn->prepare("SELECT id FROM attendance WHERE user_id=? AND event_id=? AND check_out IS NULL");
            ...
            $stmt = $conn->prepare("INSERT INTO attendance (user_id, reg_id, event_id, check_in) VALUES (?, ?, ?, NOW())");
            $stmt->bind_param("isi", $user_id, $reg_id, $event_id);

            echo json_encode([
                'success' => $stmt->execute(),
                'message' => $stmt->execute() ? 'Checked in successfully!' : 'Check-in failed.'
            ]);
            exit;
        }
        ...
        if ($type === 'out') {
            ...
            echo json_encode([
                ...
                'message' => $stmt->affected_rows > 0
                    ...
                    : 'User already checked-out / not checked-in.'
            ]);
            exit;
        }
        exit;
