...
async function loadUsers() {
  ...
  if (!tableBody) return;
  tableBody.innerHTML = "<tr><td colspan='10'>Loading...</td></tr>"; // Matches 10 columns
  ...
  tableBody.querySelectorAll(".btn-edit").forEach((btn) => {
    btn.addEventListener("click", async (ev) => {
      ...
      if (r.success && r.data) {
        const u = r.data;
        // Placeholder: Alert for now; redirect to users.html for editing if needed
        alert(`Edit user: ${u.name} (ID: ${u.id})`);
      } else {
